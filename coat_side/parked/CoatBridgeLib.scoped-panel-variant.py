...
class CoatBridgePanel(object):
    ...
    def ui(self):
        ...
        items.append("---")
        items.append("##Export scope: selected node + descendants")
        ...
        items.append("##Sculpt geometry only; no texture export")
        items.append("---")
    ...
    def SendToBlender(self):
        ...
        try:
            names, faces = export_subtree(coat, path, self.ReductionPercent)
            write_signal(root, path)
        except Exception as exc:
            self._report("Selected subtree export failed: %s" % exc,
                         "No whole-scene fallback; select a valid Sculpt Tree node")
            return
        self._report("Queued for Blender: %s (%d objects, %d faces)" %
                     (", ".join(names), len(names), faces),
                     "Current Tree node + descendants; sculpt geometry only")
