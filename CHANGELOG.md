...
* **Upgrading is a one-restart affair, and the menu entry keeps working until it is
  replaced.**  3D-Coat builds its menus from the generated XML files when it starts and
  does not read them again in that session, so the extension rewrites the files first and
